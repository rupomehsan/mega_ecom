...
class GetBrandsByCategoryId
{
    ...
    public static function execute($slug)
    {
        try {
            ...
            $data = self::$ProductBrandModel::query()->whereExists(function ($query) use ($Category) {
                $query->select("*")
                    ->from('product_category_brand')
                    ->whereRaw('product_category_brand.product_brand_id = product_brands.id')
                    ->where('product_category_brand.product_category_id', $Category->id);
            });
